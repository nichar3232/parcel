# Oddlot current-product walkthrough

Use the review build in configured Oddlot localnet mode. Confirm the footer says **Solana local validator**. All assets and counterparties are test values.
...
7. Advance Market controls to February 7, 2025. The program settles due contracts and repays the loan using the stored expiry observation. Open Activity and inspect the confirmed Oddlot vault, revision, slot and transaction proof.
...
Recording: `ODDLOT_URL=http://127.0.0.1:13028 npm run record:demo` on the Mac uses the actual UI and Samantha narration. `python3 scripts/encode-oddlot.py .state/oddlot-media submission/oddlot-demo.mp4` muxes with ffmpeg. The encoder can run on the VPS; neither script publishes the result.
