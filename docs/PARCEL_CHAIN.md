...
## Version four: every stock in one vault

Version four (`initialize_v4` / `execute_v4`, account `LedgerV4`) holds USDC and every underlying the desk writes on — NVDA and the eight PreStocks pre-IPO tokens — in one book, with a mark, reserves and borrow parameters per stock, so the pre-IPO sleeve settles onchain exactly as NVDA does. The ledger records a six-decimal test mint per asset (`PARCEL_STOCK_MINTS=SYMBOL=mint,...`, provisioned by `ops/provision-devnet-mints.mjs`); each `execute_v4` carries the wallet/pool token-account pair of every asset the action moves between wallet and vault, and the program refuses any other asset moving. A live clock attests one mark per stock (0 keeps the last), and settlement closes are keyed by expiry and stock. The replay tables are compiled per stock from `data/`. Version-three ledgers are not readable by version four and live at different derived addresses. Devnet was upgraded in place on 2026-09-26 (`2gfH6yJsGevRMwKMm2kfFCGeTmTPoZ8UZGupKS2rShU5nWRYZcvr2R3oNSjP3HuKGByuPp1rkN1SSP8JF33jaJco`, then `HtKjm9bHhVLutFaNepEyPcP6smDYY2mBEodce1E3FCN4zMPg9yueAKExotmEZFPK7FucJXZnWp968wRe5MgGHcK`). `scripts/verify-parcel-adversarial.ts` still builds version-three transactions and needs porting.

## Operator configuration

Set `PARCEL_CHAIN_ENABLED=true`, `PARCEL_PROGRAM_ID`, `PARCEL_CASH_MINT`, and `PARCEL_STOCK_MINTS` (a `SYMBOL=mint` pair for every underlying), alongside the existing `CHAIN_ENABLED=true`, an RPC and an exact `SOLANA_GENESIS_HASH` pin.
