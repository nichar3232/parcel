...
A cash loan draws USDC from an asset's test pool against stock left in the vault. The pledge is reserved the way shares under a covered call are: it cannot be withdrawn, sold or lent while the loan is out, and it never nets against another obligation. The draw is capped at the asset's loan-to-value. The rate comes off the reserve's two-slope curve at the pool's utilisation: a fixed loan stores the rate it opened at and settles on its last session, from vault cash if there is enough and otherwise by selling the pledge at that session's mark; a variable loan stores the rate again every session and runs until repaid. Interest is a claim settled at close, not an escrow, so drawn cash stays spendable. At every session, a loan whose principal and interest exceed the pledge's value at the asset's liquidation threshold is sold up at that session's mark, and the pool takes the asset's liquidation bonus from the proceeds. Pool utilisation is the simulated maker's; the stored rate is what the ledger replays.
