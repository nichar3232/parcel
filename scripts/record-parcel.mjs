...
const base = process.env.PARCEL_URL;
if (!base) throw Error('Set PARCEL_URL to the review server.');
const media = path.resolve('.state/parcel-media');
...
const scenes = [
  [
    ...
    async () => {
      ...
      await expect(page.locator('.parcel')).toHaveAttribute(
        'data-ready',
        'true',
      );
  ...
  [
    ...
    async () => {
      ...
      await writeFile(
        'submission/parcel-ui-evidence.json',
        JSON.stringify(
          {
...
console.log('Verified recording ready for encode-parcel.py.');
