...
export function SizingControl({
  ...
  const calculate = async () => {
    ...
    try {
      ...
      onApply(r.terms);
      setMessage(
        `${qty(r.terms.quantity)} share-equivalents, ${usd(r.premium, 6)} premium, ${usd(r.cashFunding, 6)} standalone cash funding plus ${qty(r.shares)} shares. Estimated P&L per +1¢: ${usd(r.centSensitivity, 6)}.${r.limited ? ' Maximum size reached.' : ''}`,
      );
    } catch (e) {
    ...
    <details className="od-sizing">
      ...
      <div className="od-form-grid">
        <Field label="Size by">
          ...
          >
            <option value="premium">Premium budget in USDC</option>
