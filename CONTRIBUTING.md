...
- `app/` and `components/desk/` render the desk. Keep wallet, ledger and settlement decisions out of components.
- `hooks/desk/` coordinates UI requests using the shared types in `lib/contracts/api.ts`.
- `server/domain/` owns authorization, validated actions, revisions and durable operations. `server/db/` owns SQL; `server/solana/` owns RPC and instruction serialization.
- `lib/` contains shared terms, historical scenarios and arithmetic. The server is authoritative even when the client previews a payoff.
