/*
 * Parcel as MCP tools. An agent drives the same HTTP API the desk uses, so
 * every action goes through server-authoritative accounting and, in chain
 * mode, the same signed transactions.
 *
 * PARCEL_URL      the running app (default http://localhost:3025)
 * PARCEL_SESSION  optional strata_session cookie value; set it to the
 *                 browser's session so the desk shows what the agent does
 */
...
const base = (process.env.PARCEL_URL || 'http://localhost:3025').replace(
  /\/$/,
  '',
);
const borrowed = Boolean(process.env.PARCEL_SESSION);
let token = process.env.PARCEL_SESSION || '';
...
async function call(path: string, init: RequestInit = {}) {
  ...
  if (token) headers.set('Cookie', `strata_session=${token}`);
...
const slot = () =>
  join(
    tmpdir(),
    `parcel-mcp-${createHash('sha256').update(token).digest('hex').slice(0, 16)}.json`,
  );
